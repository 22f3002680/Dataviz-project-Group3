...
This report is generated by `scripts/build_order_journey_eda.py` from
`Dataviz_proj_all_datasets.xlsx`. The analysis is order-level unless stated
otherwise. It is intended to establish the marketplace baseline before the
regional, product, seller, and delivery-impact analyses.

## Scope and data handling

- Source: `orders_dataset` and `order_reviews_dataset` workbook sheets.
...
- No orders are removed for this baseline overview. Missing timestamps and missing reviews are reported through the relevant denominator rules.
...
| Completed delivery records | 96,470 (97.0% of orders) |
...
This chart uses the project definition of a completed delivery, rather than
assuming that the `delivered` status alone guarantees a usable timestamp.
