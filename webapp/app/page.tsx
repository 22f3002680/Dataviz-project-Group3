  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      <header className="sticky top-0 z-10 border-b" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
        <div className="mx-auto max-w-[1400px] px-5 py-3 flex items-center gap-4 flex-wrap">
          <div>
            <h1 className="text-[15px] font-semibold tracking-tight">Group 3 · Marketplace Weekly Dashboard</h1>
            <p className="text-[11px]" style={{ color: "var(--muted)" }}>Brazilian e-commerce · grow without breaking the customer experience</p>
          </div>
        ...
        {/* Tabs */}
        <div className="mx-auto max-w-[1400px] px-5 flex gap-1">
          {(["overview", "product", "seller"] as Tab[]).map((x) => (
            <button key={x} onClick={() => setTab(x)}
              className="px-3 py-2 text-[13px] capitalize border-b-2 -mb-px transition-colors"
              style={{ borderColor: tab === x ? "var(--accent)" : "transparent", color: tab === x ? "var(--text)" : "var(--muted)" }}>
              {x}
            </button>
          ))}
        </div>
      ...
      <main className="mx-auto max-w-[1400px] px-5 py-4 space-y-4">
        {/* KPI band (all tabs) */}
        ...
        {tab === "overview" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card title="Weekly order volume" hint="cumulative up to selected week; peak annotated">
              {ov && <EChart height={260} option={volumeLine(ov.volume, t, week)} />}
            </Card>
            <Card title="Orders by state" hint="click a state to filter the dashboard">
              {ov && <EChart height={260} onEvents={stateClick}
                option={horizontalBar((ov.byState as Row[]).map((d) => ({ name: String(d.state), value: Number(d.orders) })), t, { top: 12 })} />}
            </Card>
            <Card title="Late-delivery rate by state" hint="rate, not volume (population-independent) · click to filter">
              {ov && mapReady && <EChart height={320} onEvents={stateClick} option={choropleth(ov.lateByState, t)} />}
            </Card>
            <div className="grid grid-rows-2 gap-4">
              <Card title="Top / bottom 3 categories" compact>
                {ov && <MiniTable head={["", "Category", "Orders", "Review"]}
                  rows={[...tag(ov.topCats, "▲"), ...tag(ov.bottomCats, "▼")].map((d: Row) => [d.mark, d.name, d.orders, d.avg_review])} />}
              </Card>
              <Card title="Top / bottom 3 sellers" compact>
                {ov && <MiniTable head={["", "Seller", "Orders", "Review"]}
        ...
        {tab === "product" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card title="Orders vs delayed by category" hint="delays highlighted" wide>
              {pr && <EChart height={300} option={mergedBar(pr.merged, t)} />}
            </Card>
            <Card title="Category review change vs prior week" hint="green improved · red declined">
              {pr && <EChart height={300} option={divergingBar(pr.reviewChange, t)} />}
            </Card>
            <Card title="Category risk quadrant" hint="revenue × review, bubble = volume; red = weak reviews">
              {pr && <EChart height={320} option={quadrantScatter(pr.quadrant, t)} />}
            </Card>
            <Card title="Categories needing intervention (this week)" compact>
              {pr && <MiniTable head={["Category", "Review", "Late", "Orders"]}
                rows={(pr.intervention as Row[]).slice(0, 8).map((d) => [d.name, d.avg_review, d.late_orders, d.orders])} />}
            </Card>
          </div>
        )}
        ...
        {tab === "seller" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card title="Orders vs delayed by intervention seller" hint="delays highlighted">
              {sl && <EChart height={300} option={mergedBar(sl.merged, t)} />}
            </Card>
            <Card title="Seller review change vs prior week" hint="green improved · red declined">
              {sl && <EChart height={300} option={divergingBar(sl.reviewChange, t)} />}
            </Card>
            <Card title="Seller revenue vs satisfaction" hint="red = intervention watchlist">
              {sl && <EChart height={300} option={scatterSellers(sl.scatter, t)} />}
            </Card>
            <Card title="Poor sellers → suggested alternative" hint="nearest healthy seller, same category (geolocation)" compact>
              {sl && <MiniTable head={["Seller", "Rev", "Late", "→ Alt", "Category", "km", "Alt rev."]}
                rows={(sl.watchlist as Row[]).slice(0, 8).map((d) => [d.seller, d.avg_review, d.late_orders, d.alt ?? "—", d.shared_category ?? "—", d.distance_km ?? "—", d.alt_avg_review ?? "—"])} />}
            </Card>
          </div>
        )}
      </main>
...
function Kpi({ label, value, sub, accent, color }: { label: string; value?: number | string | null; sub?: string; accent?: boolean; color?: string }) {
  return (
    <div className="card px-3 py-2.5">
      ...
      <div className="text-[22px] font-semibold leading-tight" style={{ color: color ?? (accent ? "var(--accent)" : "var(--text)") }}>
...
function Card({ title, hint, children, compact, wide }: { title: string; hint?: string; children: React.ReactNode; compact?: boolean; wide?: boolean }) {
  return (
    <div className={`card p-3.5 ${wide ? "lg:col-span-1" : ""}`}>
      <div className="mb-2">
        <h3 className="text-[13px] font-semibold">{title}</h3>
        {hint && <p className="text-[11px]" style={{ color: "var(--muted)" }}>{hint}</p>}
      </div>
      <div style={compact ? { maxHeight: 300, overflow: "auto" } : undefined}>{children}</div>
    </div>
  );
}
    ...
    <table className="w-full text-[12px]">
      <thead>
        <tr style={{ color: "var(--muted)" }}>
          {head.map((h, i) => <th key={i} className="text-left font-medium pb-1.5 pr-2">{h}</th>)}
        </tr>
      </thead>
      <tbody>
        {rows.map((r, i) => (
          <tr key={i} className="border-t" style={{ borderColor: "var(--border)" }}>
            {r.map((c, j) => <td key={j} className="py-1.5 pr-2 whitespace-nowrap">{c ?? "—"}</td>)}
          </tr>
        ))}
      </tbody>
    </table>
